...
class HumanDetectionSystem:
    ...
    def load_age_gender_model(self):
        ...
        # Check if model exists, if not create a dummy model for Railway
        if not os.path.exists(age_model_path):
            logger.warning(f"Age/Gender model not found at: {age_model_path}")
            logger.info("Creating dummy model for deployment...")
            
            # Create models directory if it doesn't exist
            os.makedirs('models', exist_ok=True)
            
            # Create a dummy model for Railway deployment
            Classes = 9
            ...
            logger.info("Using dummy age/gender model")
            return
        ...
        try:
            ...
            logger.error(f"Error loading age/gender model: {e}")
    ...
    def process_frame(self, frame):
        ...
        try:
            ...
            for box in boxes:
                ...
                if score < 0.25 or int(cls) != 0:  # Only process person class
                    continue
...
def generate_frames():
    ...
    if not detection_system:
        logger.error("Detection system not available")
        return
    
    # Try different camera indices for Railway
    ...
    for idx in camera_indices:
        try:
            ...
            logger.warning(f"Could not open camera {idx}: {e}")
    ...
    camera.set(cv2.CAP_PROP_FRAME_WIDTH, 640)  # Smaller resolution for Railway
    camera.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
    camera.set(cv2.CAP_PROP_FPS, 15)  # Lower FPS for Railway stability
    ...
    while is_streaming:
        try:
            ...
            encode_params = [cv2.IMWRITE_JPEG_QUALITY, 80]  # Lower quality for Railway
...
# Flask routes (same as before, but with error handling)
@app.route('/')
def index():
    """Main page."""
...
def get_fps():
    ...
    try:
        ...
        return jsonify({'fps': 0, 'frame_count': 0, 'elapsed_time': 0})
